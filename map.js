...
class Graph{
    ...
    add_edge(frm,to){
        ...
        //calculate distance
        var distance = Math.sqrt((this.vert_dict[to].latitude-this.vert_dict[frm])*(this.vert_dict[to].latitude-this.vert_dict[frm])+(this.vert_dict[to].longitude-this.vert_dict[frm].longitude)*(this.vert_dict[to].longitude-this.vert_dict[frm].longitude));
    ...
    loadGraph(){
        ...
        for(var singleRow = 0;singleRow<allRows.length;singleRow++){
            ...
            this.add_vertex(rowCells[0],rowCells[1],rowCells[2]);
        }
    ...
    /**
     * calculates the shortest path from start to end by updating the prev values of every node on the path
    ...
    shortest(v, path){
        if(v.previous!=null){
            path.append(v.previous.get_id());//add node to path
    ...
    get_closest_node_to_curr_location(latitude,longitude){
        ...
        var min_node;
        for(var v in vals){
            var temp_distance = this.get_distance(curr_location,v);
...
/*
var g = new Graph();
var v1 = new Vertex("v1",10,20);
var v2 = new Vertex("v2",30,40);
//g.add_vertex(v1);
//g.add_vertex(v2);
//g.add_edge(v1,v2);
g.load_graph();
